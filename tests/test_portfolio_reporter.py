...
async def test_portfolio_reporter():
    ...
    try:
        ...
        )  # trading_mode 파라미터 제거하면 yaml 설정 사용
        ...
        print(
            f"   계좌 요약 데이터: {'✅ 조회됨' if account_summary else '❌ 조회 실패'}"
        )
        ...
        response = (
            input("실제로 텔레그램 메시지를 전송하시겠습니까? (y/N): ").strip().lower()
        )
...
async def test_simple_messages():
    ...
    try:
        ...
        for msg_type in message_types:
            response = (
                input(f"{msg_type} 메시지를 전송하시겠습니까? (y/N): ").strip().lower()
            )
    ...
    for mode in modes:
        ...
        if response in ["y", "yes"]:
            try:
                ...
                # 전송 여부 확인
                send_response = (
                    input(f"{mode} 모드 리포트를 전송하시겠습니까? (y/N): ")
                    .strip()
                    .lower()
                )
...
async def main():
    ...
    if success:
        ...
        # 양쪽 모드 테스트
        response = (
            input("\n양쪽 모드(demo/real) 모두 테스트하시겠습니까? (y/N): ")
            .strip()
            .lower()
        )
        if response in ["y", "yes"]:
