...
# 설정파일 로딩
CONFIG_FILE = os.path.join(ka.config_root, "kis_devlp.yaml")
...
class DomesticStockTrading:
    ...
    def get_current_price(self, stock_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 현재가: {result['current_price']:,}원 ({result['change_rate']:+.2f}%)"
                )
                return result
            else:
                logger.error(
                    f"현재가 조회 실패: {res.getErrorCode()} - {res.getErrorMessage()}"
                )
                return None
    ...
    def calculate_buy_quantity(self, stock_code: str, buy_amount: int = None) -> int:
        ...
        if current_quantity == 0:
            logger.warning(
                f"[{stock_code}] 현재가 {current_price:,}원 > 매수금액 {amount:,}원 - 매수 불가"
            )
        else:
            total_amount = current_quantity * current_price
            logger.info(
                f"[{stock_code}] 매수 가능: {current_quantity}주 x {current_price:,}원 = {total_amount:,}원"
            )

        return current_quantity

    def buy_market_price(
        self, stock_code: str, buy_amount: int = None
    ) -> Dict[str, Any]:
        """
        시장가 매수
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 시장가 매수 주문 성공: {buy_quantity}주, 주문번호: {order_no}"
                )
    ...
    def buy_limit_price(
        self, stock_code: str, limit_price: int, buy_amount: int = None
    ) -> Dict[str, Any]:
        """
        지정가 매수
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 지정가 매수 주문 성공: {buy_quantity}주 x {limit_price:,}원, 주문번호: {order_no}"
                )
    ...
    def buy_closing_price(
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 시간외 종가 매수 주문 성공: {buy_quantity}주, 주문번호: {order_no}"
                )
    ...
    def buy_after_market(
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 시간외 단일가 매수 주문 성공: {buy_quantity}주, 주문번호: {order_no}"
                )
    ...
    def buy_reserved_order(
        self, stock_code: str, buy_amount: int = None, end_date: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 예약주문 매수 성공: {buy_quantity}주, {order_type_str}, {period_str}"
                )
    ...
    def sell_all_market_price(self, stock_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 시장가 전량 매도 주문 성공: {buy_quantity}주, 주문번호: {order_no}"
                )
    ...
    def sell_all_closing_price(self, stock_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 시간외 종가 매도 주문 성공: {buy_quantity}주, 주문번호: {order_no}"
                )
    ...
    def sell_all_reserved_order(
        ...
        try:
            ...
            if res.isOK():
                ...
                logger.info(
                    f"[{stock_code}] 예약주문 매도 성공: {buy_quantity}주, {order_type_str}, {period_str}"
                )
    ...
    async def async_buy_stock(
        self, stock_code: str, buy_amount: int = None, timeout: float = 30.0
    ) -> Dict[str, Any]:
        """
        비동기 매수 API (타임아웃 포함)
        ...
        try:
            return await asyncio.wait_for(
                self._execute_buy_stock(stock_code, buy_amount), timeout=timeout
            )
        ...
        async with stock_lock:  # 1단계: 종목별 동시 매매 방지
            async with self._semaphore:  # 2단계: 전체 동시 요청 수 제한
                async with self._global_lock:  # 3단계: 계좌 정보 보호
                    try:
                        logger.info(
                            f"[비동기 매수 API] {stock_code} 매수 프로세스 시작 (금액: {amount:,}원)"
                        )
                        ...
                        if not current_price_info:
                            ...
                            logger.error(
                                f"[비동기 매수 API] {stock_code} 현재가 조회 실패"
                            )
                            return result
                        ...
                        if buy_quantity == 0:
                            result["message"] = (
                                f"매수 가능 수량이 0입니다 (매수금액: {amount:,}원)"
                            )
                            logger.warning(
                                f"[비동기 매수 API] {stock_code} 매수 가능 수량 0"
                            )
                            return result

                        result["quantity"] = buy_quantity
                        result["total_amount"] = (
                            buy_quantity * current_price_info["current_price"]
                        )

                        # 3단계: 시장가 매수 실행 (amount 사용)
                        logger.info(
                            f"[비동기 매수 API] {stock_code} 시장가 매수 실행: {buy_quantity}주 x {amount:,}원"
                        )
                        ...
                        if buy_result["success"]:
                            ...
                            result["message"] = (
                                f"매수 완료: {buy_quantity}주 x {current_price_info['current_price']:,}원 = {result['total_amount']:,}원"
                            )
                            logger.info(f"[비동기 매수 API] {stock_code} 매수 성공")
                        else:
                            result["message"] = f"매수 실패: {buy_result['message']}"
                            logger.error(
                                f"[비동기 매수 API] {stock_code} 매수 실패: {buy_result['message']}"
                            )
    ...
    async def async_sell_stock(
        self, stock_code: str, timeout: float = 30.0
    ) -> Dict[str, Any]:
        """
        비동기 매도 API (타임아웃 포함)
        보유 수량 전량 시장가 매도
        ...
        try:
            return await asyncio.wait_for(
                self._execute_sell_stock(stock_code), timeout=timeout
            )
        except asyncio.TimeoutError:
            return {
                "success": False,
        ...
        async with stock_lock:  # 1단계: 종목별 동시 매매 방지
            async with self._semaphore:  # 2단계: 전체 동시 요청 수 제한
                async with self._global_lock:  # 3단계: 계좌 정보 보호
                    try:
                        logger.info(
                            f"[비동기 매도 API] {stock_code} 매도 프로세스 시작"
                        )

                        # 방어로직 1: 포트폴리오에서 보유 종목 확인
                        logger.info(
                            f"[비동기 매도 API] {stock_code} 포트폴리오 확인 중..."
                        )
                        ...
                        if not target_stock:
                            result["message"] = (
                                f"포트폴리오에 {stock_code} 종목이 없습니다"
                            )
                            logger.warning(
                                f"[비동기 매도 API] {stock_code} 포트폴리오에 없음"
                            )
                            return result
                        ...
                        logger.info(
                            f"[비동기 매도 API] {stock_code} 보유 확인: {target_stock['quantity']}주"
                        )
                        ...
                        if current_price_info:
                            result["current_price"] = current_price_info[
                                "current_price"
                            ]
                            logger.info(
                                f"[비동기 매도 API] {stock_code} 현재가: {current_price_info['current_price']:,}원"
                            )
                        ...
                        if holding_quantity <= 0:
                            result["message"] = (
                                f"{stock_code} 최종 확인 시 보유 수량이 0입니다"
                            )
                            logger.warning(
                                f"[비동기 매도 API] {stock_code} 최종 확인 시 보유수량 0"
                            )
                            return result

                        # 전량 매도 실행
                        logger.info(
                            f"[비동기 매도 API] {stock_code} 전량 매도 실행 (보유: {holding_quantity}주)"
                        )
                        ...
                        if all_sell_result["success"]:
                            ...
                            if result["current_price"] > 0:
                                result["estimated_amount"] = (
                                    result["quantity"] * result["current_price"]
                                )
                            ...
                            result["message"] = (
                                f"매도 실패: {all_sell_result['message']}"
                            )
                            logger.error(
                                f"[비동기 매도 API] {stock_code} 매도 실패: {all_sell_result['message']}"
                            )
        ...
        try:
            ...
            if res.isOK():
                ...
                if output2:
                    ...
                    logger.info(
                        f"계좌 총평가: {total_eval:,.0f}원, 총손익: {total_profit:+,.0f}원"
                    )
                ...
                logger.error(
                    f"잔고 조회 실패: {res.getErrorCode()} - {res.getErrorMessage()}"
                )
                return []
        ...
        try:
            ...
            if res.isOK():
                ...
                if output2:
                    ...
                    logger.info(
                        f"계좌 요약: 총평가 {account_summary['total_eval_amount']:,.0f}원, "
...
class AsyncTradingContext:
    ...
    async def __aenter__(self):
        self.trader = DomesticStockTrading(
            mode=self.mode, buy_amount=self.buy_amount, auto_trading=self.auto_trading
        )
        return self.trader
