...
class PortfolioTelegramReporter:
    ...
    def __init__(
        self, telegram_token: str = None, chat_id: str = None, trading_mode: str = None
    ):
        """
        초기화
        ...
        if not self.telegram_token:
            raise ValueError(
                "텔레그램 봇 토큰이 필요합니다. 환경 변수 TELEGRAM_BOT_TOKEN 또는 파라미터로 제공해주세요."
            )

        if not self.chat_id:
            raise ValueError(
                "텔레그램 채널 ID가 필요합니다. 환경 변수 TELEGRAM_CHANNEL_ID 또는 파라미터로 제공해주세요."
            )

        # 트레이딩 설정 - yaml 파일의 default_mode를 기본값으로 사용
        self.trading_mode = (
            trading_mode if trading_mode is not None else _cfg["default_mode"]
        )
        self.telegram_bot = TelegramBotAgent(token=self.telegram_token)

        logger.info(f"PortfolioTelegramReporter 초기화 완료")
        logger.info(
            f"트레이딩 모드: {self.trading_mode} (yaml 설정: {_cfg['default_mode']})"
        )
    ...
    def create_portfolio_message(
        self, portfolio: List[Dict[str, Any]], account_summary: Dict[str, Any]
    ) -> str:
        """
        ...
        if portfolio:
            ...
            for i, stock in enumerate(portfolio, 1):
                ...
                message += (
                    f"   📊 {quantity}주 | 평단: {self.format_currency(avg_price)}\n"
                )
...
async def main():
    ...
    parser.add_argument(
        "--type",
        choices=["full", "simple", "morning", "evening", "market_close", "weekend"],
        default="full",
        help="리포트 타입",
    )
