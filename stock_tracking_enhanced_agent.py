...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"enhanced_stock_tracking_{datetime.now().strftime('%Y%m%d')}.log"
        ),
    ],
)
...
class EnhancedStockTrackingAgent(StockTrackingAgent):
    ...
    async def _analyze_market_condition(self):
        ...
        try:
            ...
            # 1달 전 날짜
            one_month_ago = (dt.datetime.now() - dt.timedelta(days=30)).strftime(
                "%Y%m%d"
            )
            ...
            logger.info(
                f"시장 상태 분석 완료: {'강세장' if market_condition == 1 else '약세장' if market_condition == -1 else '중립'}, 변동성: {avg_volatility:.2f}%"
            )
    ...
    async def _dynamic_stop_loss(self, ticker, buy_price):
        ...
        try:
            ...
            adjusted_stop_loss_pct = min(
                max(base_stop_loss_pct * relative_volatility, 3.0), 15.0
            )
            ...
            logger.info(
                f"{ticker} 동적 손절가 계산: {stop_loss:,.0f}원 (변동성: {volatility:.2f}%, 손절폭: {adjusted_stop_loss_pct:.2f}%)"
            )
    ...
    async def _dynamic_target_price(self, ticker, buy_price):
        ...
        try:
            ...
            adjusted_target_pct = min(
                max(base_target_pct * relative_volatility, 5.0), 30.0
            )
            ...
            logger.info(
                f"{ticker} 동적 목표가 계산: {target_price:,.0f}원 (변동성: {volatility:.2f}%, 목표 수익률: {adjusted_target_pct:.2f}%)"
            )
    ...
    async def process_reports(self, pdf_report_paths: List[str]) -> Tuple[int, int]:
        ...
        try:
            ...
            for pdf_report_path in pdf_report_paths:
                ...
                if (
                    decision != "진입"
                    or effective_buy_score < min_score
                    ...
                    if not sector_diverse:
                        ...
                        if decision == "진입":
                            ...
                            logger.info(
                                f"매수 점수 부족으로 결정 변경: {company_name}({ticker}) - 진입 → 관망 (점수: {effective_buy_score} < {min_score})"
                            )
    ...
    async def _analyze_sell_decision(self, stock_data):
        ...
        try:
            ...
            if self.market_condition == -1 and trend < 0 and profit_rate > 3:
                return (
                    True,
                    f"약세장 + 하락 추세에서 수익 확보 (수익률: {profit_rate:.2f}%)",
                )
            ...
            if (
                (stop_loss == 0 or current_price > stop_loss)
                and profit_rate <= -5
                and trend < 1
            ):
