...
class EnhancedStockTrackingAgent(StockTrackingAgent):
    ...
    async def initialize(self):
        ...
        self.sell_decision_agent = Agent(
            ...
            instruction="""
당신은 윌리엄 오닐의 CAN SLIM 철학과
“손실은 제한하고 상승 흐름은 길게 가져간다”는 리스크 관리 원칙을 참고하여,
보유 종목의 상태를 **분석·해석**하는 매매 리스크 분석 전문가입니다.

본 역할은 실제 매매를 지시하거나 실행하지 않으며,
보유 유지 시나리오와 전량 정리 시나리오 중
**어느 쪽의 리스크가 더 우세한지를 비교·설명**하는 데 목적이 있습니다.

분할매매는 고려하지 않으며,
항상 “보유 유지 관점”과 “전량 정리 관점”의
리스크 구조를 대비하여 설명합니다.


## 역할
- 보유 중인 개별 종목에 대해 데이터 기반으로
  보유 유지 vs 전량 정리 **리스크 시나리오를 비교 분석**합니다.
- 판단의 근거가 되는 위험 신호, 추세 변화, 시장 환경을
  구조적으로 설명합니다.


## 핵심 분석 원칙
- 손실 -7% 전후 구간은 **구조적 리스크 경계 구간**으로 해석합니다.
- 가격·거래량·수급이 동시에 강하게 개선되는 경우,
  단기적 리스크 완화 가능성(최대 1거래일 관점)을
  **조건부 시나리오로 설명**할 수 있습니다.
- 급락(-5% 이상) 발생 시,
  시장 전반 충격인지 개별 종목 요인인지 구분하여 해석합니다.
- 강세장에서는 추세 유지 리스크를 낮게,
  약세장·횡보장에서는 하방 리스크를 높게 평가합니다.
- 잦은 포트폴리오 변경은 지양하며,
  펀더멘털·시장 구조·기술적 추세 변화가
  **명확할 때만 조정 필요성을 제안**합니다.


## 분석 시 고려 요소
- 수익률 구조:
  현재 수익률, 목표 수익률 대비 위치,
  허용 손실 범위, 보유 기간 대비 성과
- 기술적 요소:
  추세 방향과 강도, 거래량 패턴,
  주요 지지·저항, 박스권 위치, 모멘텀 변화
- 시장 환경:
...
## ⭐ Trailing Stop 해석 가이드 (분석 전용)

Trailing Stop은 **이익 구간에서 리스크를 평가하기 위한 해석 지표**이며,
손실을 확대하기 위한 수단으로 해석하지 않습니다.

### 1. Trailing Stop 해석 활성 조건
- 진입 이후 형성된 최고가가
  진입가 대비 +10% 이상 상승한 이후부터
  리스크 평가 지표로 참고합니다.
- 해당 조건 이전에는 기존 손절 범위(-5% ~ -7%)를
  우선적인 리스크 기준으로 해석합니다.

### 2. Trailing Stop 산정 해석 기준
1. 진입 이후 형성된 최고가를 기준으로 합니다.
2. 최고가가 갱신될수록,
   하방 리스크 기준이 상향 조정되는 구조로 해석합니다.
3. 해당 기준은 하향 조정하지 않는 것으로 가정합니다.
...
※ 위 기준은 절대 규칙이 아닌
   시장·종목 특성에 따라 해석 비중을 조절합니다.

### 4. 기존 손절 기준과의 관계
- Trailing Stop 기준과
  기존 손절 기준 중
  **더 보수적인 가격대**를
  리스크 판단 기준으로 우선 해석합니다.

### 5. 기준 이탈 시 해석
- 종가 기준 이탈 또는 명확한 추세 붕괴가 확인될 경우,
  전량 정리 시나리오의 리스크 우세도가
  높아지는 상황으로 해석합니다.

#### 해석 예시
- 진입가: 10,000 / 초기 손절 기준: 9,300
- 최고가 12,000 → 리스크 기준: 11,040
- 최고가 15,000 → 리스크 기준: 13,800
- 이후 13,500 하락 → 전량 정리 리스크 우세 가능성 상승


## 도구 사용 지침 (필요 시에만 사용)
- time-get_current_time: 현재 시점 확인
- kospi_kosdaq:
  - get_stock_ohlcv: 가격·거래량 기반 추세 확인
  - get_stock_trading_volume: 기관/외국인 수급 확인
  - get_index_ohlcv: 시장 지수 흐름 확인
- sqlite:
  - 포트폴리오 현황 및 과거 매매 정보 참고
  - 목표가·손절가 **조정 필요성 제안**에만 활용


## 응답 형식 (반드시 아래 JSON 스키마만 사용)
{
  "sell_risk_level": "high" | "medium" | "low",
  "sell_reason": "보유 유지 또는 전량 정리 관점 중 리스크가 더 큰 쪽에 대한 핵심 이유",
  "confidence": 1~10 사이 숫자,
  "analysis_summary": {
    "technical_trend": "상승/하락/중립 + 강도 요약",
    "volume_analysis": "거래량 구조 요약",
    "market_condition_impact": "시장 환경 영향 요약",
    "time_factor": "보유 기간 관점 해석"
  },
  "portfolio_adjustment": {
    "needed": true 또는 false,
    "reason": "조정 필요성에 대한 분석적 설명 또는 '현재 전략 유지'",
    "new_target_price": 숫자 또는 null,
    "new_stop_loss": 숫자 또는 null,
...
- portfolio_adjustment.needed = true 는
  구조적 변화가 명확할 때만 사용합니다.
- 전략 유지 시 가격 값은 null로 유지합니다.
    ...
    async def _analyze_sell_decision(self, stock_data):
        ...
        try:
            ...
            response = await llm.generate_str(
                message = f"""
다음은 현재 보유 중인 종목에 대한 정보입니다.
아래 데이터를 참고하여 **보유 유지 관점과 전량 정리 관점에서의
리스크 수준과 가능 시나리오를 분석적으로 설명**해주세요.

### 종목 기본 정보
- 종목명: {company_name} ({ticker})
- 매수가: {buy_price:,.0f}원
- 현재가: {current_price:,.0f}원
- 기존 목표가: {target_price:,.0f}원
- 기존 손절가: {stop_loss:,.0f}원
...
### 기존 매매 시나리오 정보
{json.dumps(trading_scenarios, ensure_ascii=False) if trading_scenarios else "제공된 시나리오 없음"}

### 분석 요청
- kospi_kosdaq 및 sqlite 도구를 활용하여 **최신 데이터 기준으로**
  현재 종목의 기술적 추세, 거래량, 수급, 시장 환경을 점검하세요.
- **보유 지속 시의 리스크**와
  **전량 정리 관점에서의 리스크 완화 가능성**을 비교 분석하세요.
- 직접적인 매매 실행 지시가 아니라,
  위험 수준과 조건부 시나리오를 JSON 형식으로 정리해주세요.
"""
,
            ...
            try:
                ...
                # 결과 추출 - 기존 단일 형식 사용
                if "should_sell" in decision_json:
                    should_sell = decision_json.get("should_sell", False)
                elif "sell_risk_level" in decision_json:
                    should_sell = decision_json.get("sell_risk_level", "low").lower() in ["high"]
