...
async def analyze_stock(
    ...
    async with app.run() as parallel_app:
        ...
        logger.info(
            f"시작: {company_name}({company_code}) 분석 - 기준일: {reference_date}"
        )
        ...
        agents = get_agent_directory(
            company_name, company_code, reference_date, base_sections
        )
        ...
        for section in all_sections:
            if section in section_reports:
                ...
                if section == "price_volume_analysis" and (
                    price_chart_html or volume_chart_html
                ):
                    final_report += "\n## 가격 및 거래량 차트\n\n"
                ...
                elif section == "company_status" and (
                    market_cap_chart_html or fundamentals_chart_html
                ):
                    final_report += "\n## 시가총액 및 기본 지표 차트\n\n"
        ...
        logger.info(
            f"Finalized report for {company_name} - {len(final_report)} characters"
        )
