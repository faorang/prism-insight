...
## 말투 규칙 (매우 중요):
- 보고서 본문은 반드시 '~한다', '~된다', '~이다', '~있다', '~함', '~임' 등 간결한 평어체(해라체, 음슴체)로 작성하세요.
- '~입니다', '~합니다' 등 불필요한 높임말(합쇼체) 사용을 금지합니다. (토큰 절약 및 LLM 가독성 향상 목적)
- 예시: "상승세를 보이고 있습니다" (X) → "상승세를 보인다" 또는 "상승세임" (O)
- 예시: "주목할 필요가 있습니다" (X) → "주목할 필요가 있다" 또는 "주목 필요" (O)
...
- 보고서 본문은 반드시 '~한다', '~된다', '~이다', '~있다' 등 간결하고 정제된 평서문 평어체(해라체)로 작성하세요. 완결되지 않은 어투나 종결어미 사용(음슴체)을 금지하고, 불필요한 높임말(합쇼체) 사용도 금지합니다. (토큰 절약 및 LLM 가독성 향상 목적)
