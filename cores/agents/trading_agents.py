...
def create_trading_scenario_agent():
    ...
    instruction = """## 시스템 제약 및 원칙
        ...
        - 피벗 버퍼 예외: 강력한 모멘텀/공시가 동반되어 장대양봉 혹은 상한가 진입 시도가 뚜렷하고 추가 상방 여력이 클 경우, 피벗 돌파 허용 기준을 기존 +5%에서 최대 +8%까지 완화하여 매수를 허용할 수 있다. 이 경우 JSON 출력의 'pivot_buffer_pct' 필드에 완화할 허용율(5.0~8.0)을 기재한다.
