...
KRX_STANDARD_SECTORS = [
    ...
    "전기가스업", "건설업", "운수창고", "통신업", "금융업", "은행",
    "증권", "보험", "서비스업", "제조업",
    
    # KOSDAQ
    "건설", "유통", "금융", "오락,문화", "통신방송서비스", "IT부품",
    "반도체", "정보기기", "소프트웨어", "컴퓨터서비스", "통신장비",
    "인터넷", "디지털컨텐츠", "음식료,담배", "섬유,의류", "종이,목재",
    "출판,매체복제", "제약", "비금속", "금속", "기계,장비", "일반전기전자",
    "의료,정밀기기", "운송장비,부품", "기타제조", "통신서비스", "방송서비스",
    
    # Miscellaneous / Fallback for backward compatibility
    "농업, 임업 및 어업", "부동산", "기타금융", "기타제조", "일반서비스",
    "IT 서비스", "운송·창고", "전기·가스", "전기·전자", "통신"
]
...
def create_trading_scenario_agent(language: str = "ko", sector_names: list = None):
    ...
    **단독 사유로 사용 금지된 표현:** "과열 우려", "변곡 신호", "추가 확인 필요", "단기 조정 가능성", "관망이 안전".
    ...
    - **오전장 (10:30~11:30 KST)**: 당일 거래량/캔들은 미완성입니다. "오늘 거래량이 약하다" 같은 확정 판단은 금지하십시오. 전일 종가/거래량 기준으로 분석하고, 당일 데이터는 추세 변화 참고용으로만 사용합니다.
...

    {
        ...
        "trading_scenarios": {
            ...
            "sell_triggers": [
                ...
                "오닐 절대 룰: 장중 현재가 기준 -7% 이상 손실 도달 시 즉시 무조건 전량 매도 (종가까지 기다리지 않음)",
...
def create_sell_decision_agent(language: str = "ko"):
    ...
    **핵심-1) 장중 즉시 손절 룰(Intraday Stop-Loss Rule):**
    - 손절가(`stop_loss`) 및 trailing-stop 판단은 **장중 현재가**를 기준으로 합니다. 종가까지 기다리지 않습니다.
    - 장중이라도 현재가가 손절가 이하에 도달하면 **즉시 전량 매도**합니다.
    - 이는 윌리엄 오닐의 원칙 — "손실은 작을 때 빠르게 자른다" — 에 따른 것입니다.
    - 오전장(KST 10:30~11:30 기준)에도 현재가가 손절가를 이탈하면 즉시 매도합니다.
    ...
    **핵심-4) 매도 신호 우선순위 (단일 기준점):**
    - Tier 1: 절대 매도 (장중 현재가 기준 손실률 ≥ -7%, 또는 현재가가 손절가 이탈). 장중 터치 즉시 매도.
    - Tier 2: Trailing-stop 현재가 이탈 (Core-3에 의해 활성화된 경우에만 적용). 장중 터치 즉시 매도.
