...
def create_trading_scenario_agent(language: str = "ko"):
    ...
    if language == "en":
        ...
        **1. 시장 환경 판단 (최우선)**
        - KOSPI 20일/60일 데이터 확인(kospi_kosdaq-get_index_ohlcv)
        - 시장 상태는 아래 4단계 중 하나로 반드시 분류
        ...
        판단:
        - 모멘텀 최우선
        - 명확한 부정요소가 없고 과열이 심하지 않다면 진입 검토.
        - 손익비 1.2+ 허용
        - 손절 -5%~-7% 이내
        ...
        ## 중요 판단 원칙
        - Strong Bull/Bull-Recovery 상태에서는 "왜 사야 하는가"를 우선 검토
        - Weak/Bear 상태에서는 "왜 사면 안 되는가"를 우선 검토
        - 시장 방향은 개별 종목보다 우선
        - 중기 추세를 중요하게 평가하되, 단기 과열이 심한 경우 신규 진입은 보수적으로 판단.
        - 강한 업종 + 강한 수급 동반 종목 우선
        - 최근 단기 급등 종목은 초기 돌파 구간인지 후기 과열 구간인지 반드시 구분
        - 현재가가 buy_limit_price를 2~3% 이상 초과하면 원칙적으로 미진입 우선 검토

        **2. 밸류에이션 및 종목 평가 (1~10점)**
        - time-get_current_time으로 현재 날짜 확인 후 perplexity-ask로 동종업계 비교.
        - 8~10점: 적극진입 (저평가+모멘텀+시장우호)
        - 7점: 기본 진입
        - 6.5점: Strong Bull/Bull-Recovery 상태에서만 조건부 진입
        - 6점 이하: 미진입
        ...
        **4. 포트폴리오 관리**
        ...
        - 이미 보유 중인 종목과 높은 상관관계가 있는 종목은 감점 고려

        **5. Strong Bull/Bull-Recovery 상태에서 미진입 가능 조건**
        - 손익비 부족
        - 과열 심화:
            다음 조건 중 2개 이상 충족 시 "과열 경고"로 판단:
            - RSI 72 이상
            - 최근 10거래일 +35% 이상 상승
            - 볼린저 상단 지속 이탈
            - 투자주의/경고 지정
            ...
            과열 경고 발생 시:
            - buy_score 감점 고려
            - Strong Bull 상태여도 미진입 가능
            - 손익비 기준 강화
            - 추격매수 보수적으로 판단
        - 외인/기관 순매도 전환
        - 손절 지지선이 지나치게 멀어 리스크 과다
