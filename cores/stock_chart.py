...
def configure_korean_font():
    ...
    if system == "Darwin":  # macOS
        ...
        for font_name in korean_font_list:
            try:
                ...
                if font_path and not font_path.endswith("afm"):
                    ...
                    logger.info(
                        f"한글 폰트 설정 완료 (이름): {font_name}, 경로: {font_path}"
                    )
                    return font_path
                    ...
                    for match_path in matching_paths:
                        if os.path.exists(match_path):
                            try:
                                ...
                                logger.debug(
                                    f"Linux 폰트 파일 접근 실패: {match_path} -> {e}"
                                )
                                continue
                            except (ValueError, TypeError) as e:
                                logger.debug(
                                    f"Linux 폰트 포맷 오류: {match_path} -> {e}"
                                )
                                continue
                            except PermissionError as e:
                                logger.debug(
                                    f"Linux 폰트 권한 오류: {match_path} -> {e}"
                                )
                                continue
                else:
                    # 일반 경로 처리
        ...
        for font_name in korean_font_names:
            try:
                ...
                if (
                    font_path
                    and not font_path.endswith(".afm")
                    ...
                    logger.info(
                        f"한글 폰트 설정 완료 (매니저): {font_name} -> {font_path}"
                    )
                    return font_path
            except (AttributeError, KeyError) as e:
    ...
    try:
        if system == "Linux":
            ...
            if (
                os.path.exists("/etc/rocky-release")
                or os.path.exists("/etc/centos-release")
...
def get_chart_as_base64_html(
    ...
    **kwargs,
    ...
    try:
        ...
        # 이미지 형식에 따라 저장 옵션 설정
        save_kwargs = {"format": image_format, "bbox_inches": "tight", "dpi": dpi}
...
def create_price_chart(
    ticker, company_name=None, days=730, save_path=None, adjusted=True
):
    """
...
def create_market_cap_chart(ticker, company_name=None, days=730, save_path=None):
    ...
    # 시가총액 플롯
    ax.fill_between(df.index, 0, df["시가총액"], color=PRIMARY_COLORS[0], alpha=0.2)
    ax.plot(df.index, df["시가총액"], color=PRIMARY_COLORS[0], linewidth=2.5)
    ...
    if KOREAN_FONT_PROP:
        ...
        ax.set_ylabel(
            "시가총액", fontsize=12, labelpad=10, fontproperties=KOREAN_FONT_PROP
        )
    else:
    ...
    pct_change = (
        (latest_point["시가총액"] - first_point["시가총액"])
        / first_point["시가총액"]
        * 100
    )
...
def create_fundamentals_chart(ticker, company_name=None, days=730, save_path=None):
    ...
    if "DIV" in df.columns:
        ...
        if KOREAN_FONT_PROP:
            ...
            axes[2].set_ylabel(
                "수익률 (%)", fontsize=11, fontproperties=KOREAN_FONT_PROP
            )
        else:
...
def create_trading_volume_chart(ticker, company_name=None, days=730, save_path=None):
    ...
    if "순매수" in df_volume.columns:
        ...
        bars = axes[0].bar(
            ...
            bar_width,
            color=[
                PRIMARY_COLORS[i % len(PRIMARY_COLORS)]
                for i in range(len(investor_data))
            ],
            alpha=0.7,
        )
        ...
        if KOREAN_FONT_PROP:
            axes[0].set_xticklabels(
                investor_data.index, rotation=45, fontproperties=KOREAN_FONT_PROP
            )
        else:
    ...
    if KOREAN_FONT_PROP:
        ...
        axes[1].set_title(
            "Daily Cumulative Net Purchase Trend", fontsize=12, loc="left"
        )
...
def create_comprehensive_report(
    ticker, company_name=None, days=730, output_dir="charts"
):
    """
    ...
    try:
        create_fundamentals_chart(
            ticker, company_name, days, save_path=fundamentals_path
        )
    ...
    try:
        create_trading_volume_chart(
            ticker, company_name, min(60, days), save_path=volume_path
        )
...
def check_font_available():
    ...
    korean_fonts = [
        f
        for f in available_fonts
        if any(char in f for char in "가나다라마바사아자차카타파하")
    ]
