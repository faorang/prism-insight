...
class StockTrackingAgent:
    ...
    async def initialize(self):
        ...
        self.trading_agent = Agent(
            ...
            instruction="""당신은 윌리엄 오닐의 CAN SLIM 기법과 핵심 철학을 연구하여
            데이터 기반의 주식 매매 시나리오를 구성하는 분석 전문가입니다.
            직접 매매 지시를 하지 않으며, 다양한 시장 상황에 대한 가능성 있는 시나리오와
            리스크 포인트를 구조적으로 설명합니다.
            분석 시에는 기업의 내재가치를 고려하는 관점을 바탕으로 하되,
            상승 모멘텀이 확실하게 나타나는 구간에서는
            기술적 흐름을 함께 반영하여 시나리오의 해석 비중을 조절합니다.
            ...
            **key_levels 예시**:
            ...
            ## 1년 CSV 데이터 활용 원칙 (매수 에이전트 전용)

            제공된 1년치 CSV 데이터를
            **매수 적합성 판단과 점수 산정에만 제한적으로 활용**한다.

            - CSV 데이터는 추세 방향, 변동성, 거래량 변화, 수급 흐름을
            정성·정량적으로 파악하는 용도로만 사용한다.
            - CSV 데이터로부터 미래 가격을 단정적으로 예측하거나,
            구체적인 매도 시점·매도 가격을 산출하지 않는다.
            - 손절, 익절, Trailing Stop, 매도 전략은
            **별도의 매도 판단 에이전트의 영역**으로 간주한다.
            - 단기 급등락이 있더라도, 1년 데이터 내에서
            구조적 추세 변화인지 일시적 변동성인지 구분하여 해석한다.

            CSV 기반 해석 결과는
            `buy_score`, `decision`, `target_price`, `stop_loss` 산정 시
            **보조 근거로만 사용하며, 단독 근거로 판단하지 않는다.**
    ...
    async def analyze_report(self, pdf_report_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                dnoh = datetime.now().strftime('%Y%m%d')
                with open(f'{pdf_report_path}_{dnoh}', 'w', encoding='utf-8') as f:
                    f.write(report_content)
            except Exception as e:
                logger.error(f"보고서 내용 저장 중 오류: {str(e)}")
