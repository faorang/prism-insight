...
2. 하이브리드 모드: 10일 데이터로 에이전트 점수 계산 및 기준 미달(에이전트 적합도 점수 0 이하) 후보 제외
3. 최종 점수 기준 각 트리거에서 중복 및 에이전트 기준 미달 종목을 배제하고 1개씩 선택 (최대 3개)
4. 최종 선택된 종목이 3개 미만인 경우, 에이전트 적합도 기준을 통과한 남은 종목들 중에서 전체 점수 순으로 추가 채우기 수행
