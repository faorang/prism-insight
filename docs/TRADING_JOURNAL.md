...
#### 주요 기능 및 알고리즘
- **직관 일괄 재추출 (`refresh_intuitions`)**: 신규 상세 일지 건수 부족으로 표준 압축이 건너뛰어지더라도, 최근 90일 내의 누적 저널 코퍼스(최대 40건)를 일괄 취합하여 2회 이상 반복되는 매매 패턴(직관)을 안정적으로 생성하고 누적합니다.
- **저널 영향도 성과 분석 (`_log_journal_influence_stats`)**: 매주 압축 스크립트 실행 완료 시, 과거 90일간 완료된 거래 중 저널 피드백(직관 참고 혹은 점수 조정)을 반영한 매수 건과 그렇지 않은 건의 평균 수익률 및 승률 비교 통계를 관찰 로그로 기록하여 피드백 루프의 실효성을 검증합니다.
